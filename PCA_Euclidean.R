...
barpos<-barplot(as.vector(order_weight_euc)[c(1:20, 1981:2000)],beside=T,col=c('black','gray'), names.arg = rep("",40),
...
for (i in (m-9):m){
  axis(1, at = barpos[2*i-2*980-1], label="", tck = -0.01)
  axis(1, at=barpos[2*i-2*980-1],lwd=0, line=0.5, labels=lookup[order(diag(PC1_euc)/sum(diag(abs(PC1_euc))))][i],las=2, cex.axis=0.6)
...
barpos<-barplot(as.vector(order_weight_euc_2)[c(1:10, 991:1000)],beside=T,col=c('black'), names.arg = rep("",20),
...
for (i in (m-9):m){
  axis(1, at = barpos[i-980], label="", tck = -0.01)
  axis(1, at=barpos[i-980],lwd=0, line=0.5, labels=lookup[order(diag(PC2_euc)/sum(diag(abs(PC2_euc))))][i],las=2, cex.axis=0.6)
